...
public final class Regex {
  ...
  public func isMatch(_ source: String, flags: EFlags = []) -> Bool {
    !matches(source, count: 1, startAt: 0, max: 1, flags: flags).isEmpty
  }
  ...
  /**
   ...
   - Returns: The found matches.
   */
  public func matches(_ source: String, maxMatches: Int = Int.max,
                      flags: EFlags = []) -> [Substring] {
    matches(source, count: 1, startAt: 0, max: maxMatches, flags: flags)
  }
  ...
  public func groups(_ source: String,
                     ...
                     flags: EFlags = []) -> [Substring] {
    matches(source, count: maxGroups, startAt: 1, max: maxMatches, flags: flags)
  }
  ...
  private func matches(_ source: String,
    ...
    for _ in 0 ..< max {
      ...
      let result = regexec(&compiledPattern, String(string), elements.count, &elements,
                           flags.rawValue)
      ...
      for element in elements[index ..< elements.count] where element.rm_so != -1 {
        let startIndex = utf8.index(utf8.startIndex, offsetBy: Int(element.rm_so))
          .samePosition(in: source)!
        let endIndex = utf8.index(utf8.startIndex, offsetBy: Int(element.rm_eo))
          .samePosition(in: source)!
      ...
      let startIndex = utf8.index(utf8.startIndex, offsetBy: Int(elements[0].rm_eo))
        .samePosition(in: source)!
